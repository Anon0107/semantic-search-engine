...
def get_prompt(query, documents):
    ...
    """
    sources = '\n\n---\n\n'.join(documents)
    prompt = f"""<instruction>You are a helpful assistant. Answer ONLY using the context given. DO NOT assume anything, always flag uncertainty when answering. Respond I don't know if answer not in context.</instruction>
...
def get_answer(prompt):
    ...
    response = anthropic_client.messages.create(
        ...
        max_tokens = 1024,
        system = 'You are a helpful assistant that answers only using given context',
